...
exports.onFollowUser = functions.firestore //done //done
...
exports.onUnfollowUser = functions.firestore //done //done
  .document("/followers/{userId}/userFollowers/{followerId}")
...
exports.onUploadPost = functions.firestore //done //done
  ...
  .onCreate(async (snapshot, context) => {
    ...
    //adding post to global feed
    admin.firebase().collection('All Posts').doc(postId).set(snapshot.data());
...
exports.onUpdatePost = functions.firestore //done //done
  .document("/posts/{userId}/userPosts/{postId}")
  .onUpdate(async (snapshot, context) => {
    ...
    //adding post to global feed
    const postsRef =
    admin.firebase().collection('All Posts');
    const postsDoc = await postsRef.doc(postId).get();
    if (postsDoc.exists){
      postsDoc.ref.update(newPostData);
    }
  });

exports.onDeletePost = functions.firestore //done //done
  ...
  .onDelete(async (snapshot, context) => {
    ...
    //delete post to global feed
    admin.firebase().collection('All Posts').doc(postId).delete();
  });

exports.addChatMessage = functions.firestore //done //done
...
exports.onNewActivity = functions.firestore //done //leaving for now
...
exports.onCreateUser = functions.auth.user().onCreate((user) => { //done //done
...
exports.onUpdateUser = functions.firestore //done //done
...
const disableUser = (userId, bool) => { //done //done
  functions.app.admin
    .auth()
